...
# Import trunc_normal_ with fallback
try:
    ...
    try:
        ...
        try:
            ...
            # Fallback: define trunc_normal_ if not available
...
try:
    ...
    try:
        ...
        # Fallback: simple to_2tuple implementation
        def to_2tuple(x):
...
# Import helper functions - try new path first, then old path
try:
    ...
    try:
        ...
        # If not available, define minimal fallbacks
        def build_model_with_cfg(*args, **kwargs):
            raise NotImplementedError("build_model_with_cfg not available")
        def resolve_pretrained_cfg(*args, **kwargs):
            return None
        def named_apply(fn, module):
            for name, child in module.named_children():
                fn(child)
                named_apply(fn, child)
        def adapt_input_conv(*args, **kwargs):
            return None
        def checkpoint_seq(*args, **kwargs):
