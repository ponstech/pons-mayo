...
def train():
    ...
    if(1):
        print('pretrained model: '+'vit_'+opt.scale+'_patch'+str(opt.patch_size)+'_'+str(opt.res))
        if(opt.scale == 'huge'):
    ...
    for epoch in range(opt.epochs):
        ...
        for i,(te_transformed_normalized_img, te_labels) in enumerate(te_dataloader):
            ...
            with torch.no_grad():
                ...
                #here editted
                topk_val = min(5, opt.num_classes)
                _, topk_pred = outputs.topk(topk_val, dim=1, largest=True, sorted=True) 

                # Check if te_labels are in the top 5 predictions
                correct_top5 += (topk_pred == te_labels.view(-1, 1).to(device)).sum().item()
            ...
            if i % 5 == 0:  #opt.log_step
                ...
                print('Test Epoch: [{0}][{1}/{2}]\t'                 
                    ...
                    'Top 5 Accuracy {top5.val:.3f} ({top5.avg:.3f})\t'.format(epoch, i, len(te_dataloader),
                                                                                ...
                                                                                top1=accs,
                                                                                top5=accs_top5))
        writer.add_scalar('classification_test_loss', epoch_te_loss.avg, epoch)
        ...
        if( accs.avg > last_th):
            ...
            if(opt.noise_type == 'gaussian'):
                torch.save(noise_vit.state_dict(),  opt.model_saved_path + '/'+'acc_'+ str(last_th)+'_'+opt.noise_type+'_NV.pkl') 
            else:
                if(opt.OptimalQ == 1):
                    torch.save(noise_vit.state_dict(),  opt.model_saved_path + '/'+'acc_'+str(last_th)+'_'+opt.noise_type+'_NoisyViT.pkl') 
                else:
                    torch.save(noise_vit.state_dict(),  opt.model_saved_path + '/'+'acc_'+str(last_th)+'_'+opt.noise_type+'_OrdinaryViT.pkl') 
