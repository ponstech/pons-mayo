import tqdm
from Multimodaldataloader import *
...
from parameters import *
import torch
from MultimodalAccdataloader import *
from utils import *
import timm
...
if(opt.OptimalQ):
    writer = SummaryWriter('/content/drive/MyDrive/NoisyNN/NoisyViT-OptimalQ/saved_models/output/Multimodal/optimal')
else:
    writer = SummaryWriter('/content/drive/MyDrive/NoisyViT-OptimalQ/saved_models/output/Multimodal/linear_'+str(int(10*opt.strength)))

cuda = True if torch.cuda.is_available() else False
device = torch.device('cuda' if cuda else 'cpu')

# Multi-modalite için transform (her modaliteye aynı uygulanacak)
transform_multimodal = transforms.Compose([
...
def train():
    ...
    if opt.datasets == 'MultiModal':
    # roots sözlüğü: modalite adı -> kök dizin
      roots = {
          'bmode': opt.bmode_path,  # parameters.py'ya eklenmeli
          'enhanced': opt.enhanced_path,
          'improved': opt.improved_path,
      }
      tra_dataloader, te_dataloader, train_dataset, test_dataset = get_multimodal_loaders(roots, opt.batch_size)
      print("Train class index map:", train_dataset.class_to_idx)
      print("Test class index map:", test_dataset.class_to_idx)
          
    elif opt.datasets == 'ImageNet':
        train_data = datasets.ImageFolder(
            root=os.path.join(opt.imagenet_path, 'train'),
            transform=transform_train
        )
        test_data = datasets.ImageFolder(
            root=os.path.join(opt.imagenet_path, 'test'),
            transform=transform_test
        )

        print("Train class index map:", train_data.class_to_idx)
        print("Test class index map:", test_data.class_to_idx)
        ...
        #test_data = datasets.ImageFolder(root=args.test_data_path, transform=transform_test) # burası eklendi
    ...
    for epoch in range(opt.epochs):
        ...
        for i,(te_transformed_normalized_img, te_labels) in enumerate(te_dataloader):
            ...
            with torch.no_grad():
                ...
                all_true.extend(te_labels.cpu().numpy()) # bunlar eklendi
        ...
        plt.title("Noisy Vit-B Multi Modal Optimal Confusion Matrix - Linear Noise")
        plt.tight_layout()
        plt.savefig("/content/drive/MyDrive/NoisyViT-OptimalQ/saved_models/output/Multimodal/ViT-B/Linear_noisyvitbase_optimal_confusion_matrix_percent.png")
        plt.close()

        # Classification Report
        report = classification_report(all_true, all_pred, target_names=class_names, output_dict=True)
        report_df = pd.DataFrame(report).transpose()
        report_df.to_csv("/content/drive/MyDrive/NoisyViT-OptimalQ/saved_models/output/Multimodal/ViT-B/Linear_noisyvitbase_optimal_classification_report.csv")
