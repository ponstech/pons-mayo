...
def collect_split_dfs(base_dir: str, modality: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
	if not os.path.isdir(base_dir):
		raise FileNotFoundError(
			...
			"Expected structure: <base_dir>/(train|val|test)/<modality>/(benign|malignant) OR <base_dir>/(train|val|test)/(benign|malignant)"
		)
	...
	if os.path.isdir(modality_path):
		...
		# Structure: train/benign/ (no modality subdirectory) - FALLBACK
		using_fallback = True
		if modality != "bmode":
			import warnings
			warnings.warn(
				f"⚠️  WARNING: Modality '{modality}' directory not found at {modality_path}. "
				f"Falling back to simple structure (same images as bmode). "
				f"This means you're using the SAME images for '{modality}' as for 'bmode'! "
				f"If you want true multi-modal analysis, ensure {modality_path} exists with separate images.",
				UserWarning,
				stacklevel=2
			)
		train_benign = load_image_paths_and_labels(os.path.join(base_dir, "train", "benign"), 0)
		train_malign = load_image_paths_and_labels(os.path.join(base_dir, "train", "malignant"), 1)
		val_benign = load_image_paths_and_labels(os.path.join(base_dir, "val", "benign"), 0)
		val_malign = load_image_paths_and_labels(os.path.join(base_dir, "val", "malignant"), 1)
		test_benign = load_image_paths_and_labels(os.path.join(base_dir, "test", "benign"), 0)
		test_malign = load_image_paths_and_labels(os.path.join(base_dir, "test", "malignant"), 1)
	else:
		raise FileNotFoundError(
			f"Neither structure found in {base_dir}. Expected either:\n"
			f"  - {os.path.join(base_dir, 'train', modality, 'benign')} (with modality)\n"
			f"  - {os.path.join(base_dir, 'train', 'benign')} (without modality)"
		)
