...
def train_gcn(model_name: str, train_graph, val_graph, hidden_channels: int, lr: float, weight_decay: float, epochs: int, patience: int, aggregation_threshold: float, save_dir: str, device: torch.device, plot_path: Optional[str] = None, early_stopping_metric: str = "accuracy") -> str:
	...
	for epoch in range(epochs):
		...
		with torch.no_grad():
			...
			val_loss = criterion(vout, val_graph.y).item()
		...
		if is_better:
			...
			if early_stopping_metric == "loss":
				print(f"Saved best GCN ({model_name}) with Val Loss: {best_val_metric:.4f}")
			else:
				print(f"Saved best GCN ({model_name}) with Val Acc: {best_val_metric:.4f}")
		else:
			counter += 1
			if counter >= patience:
				print(f"Early stopping GCN training (no improvement in val {early_stopping_metric} for {patience} epochs)")
				break
	...
	if plot_path:
		...
		if early_stopping_metric == "loss":
			...
			ax2.set_ylabel("Loss")
			ax2.set_title("GCN Train/Val Loss")
		else:
