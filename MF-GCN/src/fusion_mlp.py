...
def train_mlp_and_save(train_loader: DataLoader, val_loader: DataLoader, input_dim: int, hidden_dim1: int, hidden_dim2: int, lr: float, momentum: float, weight_decay: float, epochs: int, patience: int, save_dir: str, device: torch.device, plot_path: Optional[str] = None, early_stopping_metric: str = "accuracy") -> str:
	...
	for epoch in range(epochs):
		...
		if is_better:
			...
			if early_stopping_metric == "loss":
				print(f"Saved best MLP with Val Loss: {best_val_metric:.4f}")
			else:
				print(f"Saved best MLP with Val Acc: {best_val_metric:.2f}%")
		else:
			counter += 1
			if counter >= patience:
				print(f"Early stopping MLP training (no improvement in val {early_stopping_metric} for {patience} epochs)")
				break
