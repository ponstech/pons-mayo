...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, out_dic in enumerate(metric_logger.log_every(self.data_loader, self.log_step, f'Epoch:[{epoch}]')):
            ...
            texture, targets, name = out_dic['texture'], out_dic['label'], out_dic['name']
           ...
           # targets = targets.argmax(dim=1)
            ...
            with torch.autocast("cuda"):
                outputs = self.model(texture, self.config.channel_mixing)
            ...
            metric_logger.update(loss=loss_value)   #burada loss.item dediğimiz için benzeri olusturulmus oldu cpu için, collect sildiğimiz için ek cpu ya tasımaya gerek kalmadı valdeki gibi
        ...
        with torch.no_grad():
            ...
            for batch_idx, out_dic in enumerate(metric_logger.log_every(self.val_data_loader, 200, f'Test [{epoch}]')):
               ...
               # targets = torch.as_tensor(targets, dtype=torch.long).squeeze()
                ...
                # compute output
                output = self.model(texture, self.config.channel_mixing)
               # print("OUTPUT******************************************", output)
